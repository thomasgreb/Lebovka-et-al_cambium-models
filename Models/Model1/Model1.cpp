...
void Model1::OnDivide(ParentInfo *parent_info, CellBase *daughter1, CellBase *daughter2) {
...
void Model1::SetCellColor(CellBase *c, QColor *color) {
	// add cell coloring rules here

    double blue = c->Chemical(1)/(par->i3 + c->Chemical(1));
    double green = 0; // c->Chemical(2)/(par->i4 + c->Chemical(2));
    ...
    */
    ...
    //double red = c->Chemical(6)/(0.01 + c->Chemical(6));          //uncomment this line to visualize the division rate
    //color ->setRgbF(red,0,0);                                     //uncomment this line to visualize the division rate

    // uncomment this section to visualize the bins
    /*
...
void Model1::CellHouseKeeping(CellBase *c) {
	...
	if (c->CellType()==0) {
		...
		// rest is cambium
			...
			if ( c->Area() > c->BaseArea() * par->rel_cell_div_threshold && c->Chemical(2) > par->k[6] ) {

				c->Divide();
			}

			//if a PXY cell grows grows bigger than certain limit it turns into xylem
            if ( c->Area() > c->BaseArea() * par->rel_cell_div_threshold * 2 && c->Chemical(2) < par->i5 ) {
...
void Model1::CelltoCellTransport(Wall *w, double *dchem_c1, double *dchem_c2) {
...
void Model1::WallDynamics(Wall *w, double *dw1, double *dw2) {
	// add biochemical networks for reactions occuring at walls here
}
void Model1::CellDynamics(CellBase *c, double *dchem) {
	// add biochemical networks for intracellular reactions here

	// chemical 6 is a trick to visualize time passed from division
	// chemical 6 has no effect on the model at all
	dchem[6]  = -0.01* c->Chemical(6) ;
	
	//xylem
	if(c->CellType()==0){

		//cle41 is degraded and is consumed by pxy binding
