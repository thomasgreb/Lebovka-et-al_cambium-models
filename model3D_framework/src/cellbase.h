...
struct ParentInfo {
  ...
  Vector polarization;
  double TRANSP0membrane;
  double TRANSP0endosome;
};
...
class CellBase :  public QObject, public Vector 
{
 ...
 public:
  ...
  double CalcLumenArea(void) const;
  double CalcLigniArea(void) const;

  double RecalcArea(void) { return area = CalcArea(); }
  double RecalcLumenArea(void) { return lum_area = CalcLumenArea(); }
  ...
  double ExactLumenCircumference(void);
  double ExactLigniCircumference(void);
  inline int Index(void) const { return index; }
  inline int Parent_Index(void) const { return par_index; }
  ...
  inline double LambdaLength(void) { return lambda_celllength; }
  ...
  inline double NNodes(void) const {
    double n=0.;
  ...
  inline double NWalls(void) const {
    double n=0.;
  ...
  list<Node *> cellnodes(void){
    list<Node *> toReturn = nodes;
    return toReturn;
   }
...
inline Vector TRANSP0dir(CellBase *here, CellBase *nb, Wall *w)
{
 return w->getTransporter( here, 0)  *  w->getInfluxVector(here);
}
