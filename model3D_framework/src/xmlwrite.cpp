...
void Cell::XMLAddCore(xmlNodePtr xmlcell) const {
  ...
  }
  {
    ostringstream text;
    text << par_index;
    xmlNewProp(xmlcell, BAD_CAST "parent", BAD_CAST text.str().c_str());
  }
  ...
  {
    ostringstream text;
    text << bool_name(TRANSP0_fixed);
    xmlNewProp(xmlcell, BAD_CAST "TRANSP0_fixed", BAD_CAST text.str().c_str());
  }
...
int Cell::XMLRead(xmlNode *cur)
{
  ...
  {
    xmlChar *v_str = xmlGetProp(cur, BAD_CAST "TRANSP0_fixed");

    if (v_str==0) {
      unique_warning("Token \"TRANSP0_fixed\" not found in xmlwrite.cpp at or around line no. 1086");
    }
    if (v_str != NULL) {
      TRANSP0_fixed = strtobool( (char *)v_str);
      xmlFree(v_str);
    }
  }
...
void Mesh::XMLReadSimtime(const xmlNode *a_node)
{
  ...
  if (strsimtime) {
    ...
    time = simtime;
#ifdef QDEBUG
    qDebug() << "Simtime = " << strsimtime << "\n";
#endif
  } else {
#ifdef QDEBUG
    qDebug() << "No simtime found in file." << "\n";
#endif
    time =0;
  }	
}
  ...
  while (cur!=NULL) {
	...
	Wall::WallType wall_type;
	{
	  ...
	  if (v_str != 0) {
	    ...
	    if (!xmlStrcmp(v_str, (const xmlChar *)"subst_source")) {
	      wall_type = Wall::SubstSource;
	    } else {
	      if (!xmlStrcmp(v_str, (const xmlChar *)"subst_sink")) {
		wall_type = Wall::SubstSink;
	      } else {
