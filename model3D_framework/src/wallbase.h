...
class WallBase {
 ...
 protected:
  ...
  enum WallType {Normal, SubstSource, SubstSink};
  static int nwalls;
 protected:
  int wall_index;
  WallType wall_type;
 ...
 public:
  ...
  //! Return true if the WallBase is a source of Substance
  inline bool SubstanceSource(void) const { return wall_type == SubstSource; }
  inline bool SubstanceSink(void) const { return wall_type == SubstSink; }

  inline bool WallTypeP(void) {return wall_type;}

  inline void cycleWallType(void) { 

    if (wall_type == Normal)  
      wall_type = SubstSource;
    else
      if (wall_type == SubstSource) 
	wall_type = SubstSink;
      else
	if (wall_type == SubstSink) {
	  wall_type = Normal;
	}
  }
  ...
  inline double getthickness(void) const {return thickness; }
  inline void thicken(double new_thickness) {thickness += new_thickness; }
