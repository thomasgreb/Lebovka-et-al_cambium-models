...
bool Wall::CorrectWall( void ) {
...
#ifdef QDEBUG
      qDebug() << "Wall::CorrectWall says: Wall has three owners, but none of them is the BoundaryPolygon. I have no clue what to do with this case... Sorry!" << "\n";
      cerr << "Wall: " << *this << "\n";
      qDebug() << "Owners are: ";
  ...
  if (wall_owners.size() == 1) {
    qDebug() << "Corner point. Special case." << "\n";
  }
#endif
...
#ifdef QDEBUG
	  qDebug() << "Warning, cell wall was not corrected." << "\n";
#endif
	  return false;
	}
      }
...
string Wall::WallTypetoStr(const WallType &wt) const {
  ...
  if (wt == SubstSource) {
    return string("subst_source");
  }

  if (wt == SubstSink) {
    return string("subst_sink");
  }
