...
xmlNode *MainBase::XMLSettingsTree(void) {
  ...
  {
    xmlNode *xmloption = xmlNewChild(xmlsettings, NULL, BAD_CAST "setting", NULL);
    xmlNewProp(xmloption, BAD_CAST "name", BAD_CAST "show_cell_ligni");
    ostringstream text;
    text << bool_name(showcelllignip);
    xmlNewProp(xmloption, BAD_CAST "val", BAD_CAST text.str().c_str());
  }
  {
    xmlNode *xmloption = xmlNewChild(xmlsettings, NULL, BAD_CAST "setting", NULL);
    xmlNewProp(xmloption, BAD_CAST "name", BAD_CAST "show_cell_thick");
    ostringstream text;
    text << bool_name(showcellthickp);
    xmlNewProp(xmloption, BAD_CAST "val", BAD_CAST text.str().c_str());
  }
  {
    xmlNode *xmloption = xmlNewChild(xmlsettings, NULL, BAD_CAST "setting", NULL);
...
int MainBase::Save(const char *fname, const char *format, int sizex, int sizey)
{
  ...
  if (!QString(format).contains("PDF", Qt::CaseInsensitive)) {
    ...
    //canvas.render(painter, QRectF(), QRectF(100,0, 200, 199));
    canvas.render(painter, QRectF(), QRectF(-700,-700, 1300, 1300));
#ifdef QDEBUG
    qDebug() << "Native Image Filename: " << QDir::toNativeSeparators(QString(fname)) << "\n";
#endif
    ...
    canvas.render(&painter, QRectF(), QRectF(-1000,-1000, 1000, 1000)); // NB Fiddle with these arguments if relative size of PDF images is important.
