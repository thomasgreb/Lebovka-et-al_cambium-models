...
WallItem::WallItem( Wall *w, int wallnumber, QGraphicsScene *canvas )
  ...
  // line with TRANSP0 is a bit inside the cell wall
...
void WallItem::setColor(void) {
  ...
  Wall *w=&getWall();
  //double tr = wn==1?w->Transporters1(2):w->Transporters2(2);
  CellBase *c = wn==1?w->C1():w->C2();
//diffcolor.setRgb( (int)( ( tr / (1 + tr) )*255.), 0, 0);
//diffcolor.setHsv( (int)(   c->Index()), min(c->Chemical(2),255.0), 255);
  diffcolor.setRgb( (int)(   c->BoundaryPolP()*255), 0,255);
  if (w->SubstanceSource() && c->BoundaryPolP()) {
    setPen( QPen(purple , par.outlinewidth) );
  } else {
    if (w->SubstanceSink() && c->BoundaryPolP()) {
...
void WallItem::OnClick(QMouseEvent *e) {
  ...
  Wall *w=&getWall();
#ifdef QDEBUG
  qDebug() << "Wall ID = " << w->Index() << ", this = " << w << "\n";
  qDebug() << "Wall item = " << this << "\n";
  qDebug() << "C1 = " << w->C1()->Index() << ", C2 = " << w->C2()->Index() << "\n";
  qDebug() << "N1 = " << w->N1()->Index() << ", N2 = " << w->N2()->Index() << "\n";
#endif
  CellBase *c = wn==1?w->C1():w->C2();
  ...
  if (e->button() == Qt::RightButton) {
    QString message;
    if (wn==1) {
      message=QString("Transporter 1 = %1, color = %2, length = %3\n").arg(w->Transporters1(0)).arg(pen().color().red()).arg(getWall().Length());
    } else {
      message=QString("Transporter 2 = %1, color = %2, length = %3\n").arg(w->Transporters2(0)).arg(pen().color().red()).arg(getWall().Length());
    }
    ...
    if (e->button() == Qt::LeftButton) {
      ...
      if (e->modifiers() == Qt::ControlModifier) {
	    ...
	    if (c->BoundaryPolP()) {
	      ...
	      //cerr << "Setting PIN1\n";
	      wn==1?w->setTransporters1(0,10):w->setTransporters2(0,10);
	    }
	  } else if (e->modifiers() ==Qt::ControlModifier+Qt::ShiftModifier) {
	    wn==1?w->setTransporters1(0,0):w->setTransporters2(0,0);					
	  } else {
