...
CellBase::CellBase(const CellBase &src) :  QObject(), Vector(src)
{
    ...
    TRANSP0_fixed = src.TRANSP0_fixed;
    stiffness = src.stiffness;
    marked = src.marked;
    dead = src.dead;
    cell_type = src.cell_type;
    par_index=src.par_index;
...
ostream &CellBase::print(ostream &os) const
{
    ...
    os << " ] ";
    os << "div_counter = " << div_counter << "\n";
    os << "cell_type = " << cell_type << "\n";
    os << "\n";
    return os;
}
...
double CellBase::CalcLumenArea(void) const
{
  double radius=0;
  ...
  double erre = 1 - (thickness/radius);
    ...
    for (list<Node *>::const_iterator i=nodes.begin(); i!=(nodes.end()); i++) {
        ...
        if (i_plus_1==nodes.end())
            i_plus_1=nodes.begin();
        Vector lumen_position((((*i)->x - this->Centroid().x) * erre + this->Centroid().x),
                              (((*i)->y - this->Centroid().y) * erre + this->Centroid().y),
                              (0)
                             );
        Vector lumen_position_plus_1((((*i_plus_1)->x - this->Centroid().x) * erre + this->Centroid().x),
                                     (((*i_plus_1)->y - this->Centroid().y) * erre + this->Centroid().y),
                                     (0)                                                                                                     
                                    );
        loc_lum_area+= lumen_position.x * lumen_position_plus_1.y; 
        loc_lum_area-= lumen_position_plus_1.x * lumen_position.y;
    }
    double area = loc_lum_area>0?loc_lum_area/2.0:0;
   return area;
}


double CellBase::CalcLigniArea(void) const
{
    double loc_lig_area=0.;
    for (list<Node *>::const_iterator i=nodes.begin(); i!=(nodes.end()); i++) {
        list<Node *>::const_iterator i_plus_1=i; i_plus_1++;
        if (i_plus_1==nodes.end())
            i_plus_1=nodes.begin();
        ...
        Vector ligen_position((((*i)->x - this->Centroid().x) * erre + this->Centroid().x),
                              (((*i)->y - this->Centroid().y) * erre + this->Centroid().y),
                              (0)
                             );
        Vector ligen_position_plus_1((((*i_plus_1)->x - this->Centroid().x) * erre + this->Centroid().x),
                                     (((*i_plus_1)->y - this->Centroid().y) * erre + this->Centroid().y),
                                     (0)                                                                                                     
                                    );
        loc_lig_area+= ligen_position.x * ligen_position_plus_1.y; 
        loc_lig_area-= ligen_position_plus_1.x * ligen_position.y;
    }
   return fabs(loc_lig_area)/2.0;
}
...
double CellBase::Width(void)  const
{
...
double CellBase::Length(Vector *long_axis, double *width)  const
{
    ...
    if (long_axis) {
        *long_axis = Vector(-ixy, lambda_b - ixx, 0);
        //   cerr << "ixx = " << ixx << ", ixy = " << ixy << ", iyy = " << iyy << ", area = " << area << "\n";
    }
...
double CellBase::CalcLength(Vector *long_axis, double *width)  const
{
    ...
    if (long_axis) {
        *long_axis = Vector(-ixy, lambda_b - ixx, 0);
        //   cerr << "ixx = " << ixx << ", ixy = " << ixy << ", iyy = " << iyy << ", my_area = " << my_area << "\n";
    }
...
double CellBase::ExactLigniCircumference(void)
{

    // simply sum length of all edges
    double ligen_circumference=0.;
    ...
    for (list<Node *>::const_iterator i=nodes.begin(); i!=(nodes.end()); i++) {
        ...
        double dx=(ligen_position_plus_1.x-ligen_position.x);
        double dy=(ligen_position_plus_1.y-ligen_position.y);
        double l=sqrt(dx*dx+dy*dy);
        //    f << (*i)->x << " " << (*i)->y << " " << (*i_plus_1)->x << " " << (*i_plus_1)->y << " " << l << "\n";

        ligen_circumference += l;
    }

    return ligen_circumference;
}

double CellBase::ExactLumenCircumference(void)
{

    // simply sum length of all edges
    double lumen_circumference=0.;
    double radius=0;
    for (list<Node *>::const_iterator n=nodes.begin(); n!=nodes.end(); n++) {
      Node *i=*n;
      Vector pompilio(i->x,i->y,0);
  ...
  double erre = 1 - (thickness/radius);
    for (list<Node *>::const_iterator i=nodes.begin(); i!=(nodes.end()); i++) {
        ...
        double dx=(lumen_position_plus_1.x-lumen_position.x);
        double dy=(lumen_position_plus_1.y-lumen_position.y);
        double l=sqrt(dx*dx+dy*dy);
        //    f << (*i)->x << " " << (*i)->y << " " << (*i_plus_1)->x << " " << (*i_plus_1)->y << " " << l << "\n";

        lumen_circumference += l;
    }

    return lumen_circumference;
}
