...
class Main : public Q3MainWindow, public MainBase {
 ...
 public:
  ...
  virtual bool ShowCellLigniP(void) {return view->isItemChecked(cell_ligni_id);}
  virtual bool ShowCellThickP(void) {return view->isItemChecked(cell_thick_id);}
 ...
 // virtual bool ShowApoplastsP(void) { return view->isItemChecked(apoplasts_id);}
  ...
  void toggleCellLigni(void);
  void toggleCellThick(void);
 ...
 private:
  ...
  int cell_ligni_id; // view->Show cell wall Ligniness
  int cell_thick_id; // view->Show cell wall THickness
