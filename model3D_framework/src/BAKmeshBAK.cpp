...
nodes[first_node+ (nnodes+i-1)%nnodes],
nodes[first_node+ (i + 1)%nnodes]);
//list<Neighbor>::iterator j, jj;
//for (j = nodes[first_node]->owners.begin(); j!=nodes[first_node]->owners.end(); j++){
//  jj = j;
//}
// if (i<nnodes-1){
////   for (list<Neighbor>::iterator j = nodes[first_node]->owners.begin(); j!=nodes[first_node]->owners.end(); j++) {
////       cout<<j->getCell()->Index()<<"\n";
////   }
//  Wall *aux = new Wall(nodes[first_node+i], nodes[first_node+i+1], c, jj->getCell());
//  c->AddWall(aux);
//  //aux->CorrectWall();
//}
//else
//{
//  Wall *aux = new Wall(nodes[first_node+i], nodes[first_node], c, jj->getCell());
//  c->AddWall(aux);
//  //aux->CorrectWall();
//}
}
...
list<Node *>::reverse_iterator i=it_n1;
i++;
for (; i!=it_n2; ++i) {
...
double sum_dh=0;
//double sum_thick=0;
//double sum_thick_nb1 =0;
//double sum_thick_nb2 =0;
...
//double rx= node.mcsx * (RANDOM()-0.5); // was 100.
//double ry= node.mcsy * (RANDOM()-0.5);
...
/* OLD  if (node.boundary  && boundary_polygon->MoveSelfIntersectsP(n,  new_p )) {
NEW if (node.boundary  && boundary_polygon->MoveSelfIntersectsP(&node,  new_p )) {
...
const auto old_area =  c.area;
const auto tar_area =  c.target_area;
const auto old_fact = (old_area - tar_area) / old_area;
const auto new_fact = (old_area - delta_A - tar_area) / (old_area - delta_A);
area_dh += 1000 * (new_fact * new_fact - old_fact * old_fact);
...
cout << "ehila -- length calculations!\r";
double delta_ix =
(i_min_1.x + new_p.x)
...
//cerr << "new_celllength = "  << new_celllength << "\n";
//cerr << "target_length = "  << c.target_length << "\n";
...
/* cerr << "Delta alignment = " << alignment_before - alignment_after << "\n";
cerr << "Old alignment is " << alignment_before << ", new alignment is " << alignment_after << "\n";
cerr << "Old axis is " << old_axis << ", new axis is " << norm_long_axis << "\n";
*/
alignment_dh += alignment_before - alignment_after;
/* cerr << "alignment_dh  = " << alignment_dh << "\n";
cerr << "cellvec = " << c.cellvec << "\n";*/
} else {
...
//cout << "The intersection between " << node.Index() << " and " << cit->nb1->Index() <<  " has " << (intersection_no_nb1.size()) << " elements:\t";
//foreach (int i, intersection_no_nb1){
//  cout << i << "  ";
//}
//cout << "\t";
//cout << "The intersection between " << node.Index() << " and " << cit->nb2->Index() <<  " has " << (intersection_no_nb2.size()) << " elements:\t";
//foreach (int i, intersection_no_nb2){
//  cout << i << "  ";
//}
//cout << "\n";
w1 = 0;
...
w1 += (c->cell->Index() !=-1?c->cell->Chemical(2):par.rel_perimeter_stiffness);
}
}
for (list<Neighbor>::iterator c=cit->nb2->owners.begin(); c!=cit->nb2->owners.end(); c++) {
if (std::find(intersection_no_nb2.begin(), intersection_no_nb2.end(), c->cell->Index()) != intersection_no_nb2.end()) {
w2 += (c->cell->Index() !=-1?c->cell->Chemical(2):par.rel_perimeter_stiffness);
}
}
w1 =w1/2 * par.wk6;
w2 =w2/2 * par.wk6;
//cout << node.Index()  << "t" << cit->nb1->Index()   << "\t" << w1 << " | " << node.Index()  << "t" << cit->nb2->Index()  << "\t" << w2 << "\n";
//	for (list<Neighbor>::iterator c=node.owners.begin(); c!=node.owners.end(); c++) {
//         if (!c->cell->BoundaryPolP()){
//
//           // sum_thick += c->cell->Chemical(2);
//            sum_thick = max(c->cell->Chemical(2), sum_thick) ;
//            } else {sum_thick = par.rel_perimeter_stiffness;}
//        }
//
//    sum_thick_nb1 = 0;
//	for (list<Neighbor>::iterator c=cit->nb1->owners.begin(); c!=cit->nb1->owners.end(); c++) {
//         if (!c->cell->BoundaryPolP()){
//            //sum_thick_nb1 += c->cell->Chemical(2);
//            sum_thick_nb1 = max(c->cell->Chemical(2), sum_thick_nb1) ;
//            } else {sum_thick_nb1 = par.rel_perimeter_stiffness;}
//        }
//
//    sum_thick_nb2 = 0;
//	for (list<Neighbor>::iterator c=cit->nb2->owners.begin(); c!=cit->nb2->owners.end(); c++) {
//         if (!c->cell->BoundaryPolP()){
//            //sum_thick_nb2 += c->cell->Chemical(2);
//            sum_thick_nb2 = max(c->cell->Chemical(2), sum_thick_nb2) ;
//            } else { sum_thick_nb2 = par.rel_perimeter_stiffness;}
//        }
// w1 = sum_thick + sum_thick_nb1;
// w2 = sum_thick + sum_thick_nb2;
// cout << node.Index()  << "t" << cit->nb1->Index()   << "\t" << w1 << " | " << node.Index()  << "t" << cit->nb2->Index()  << "\t" << w2 << "\n";
//double mecis = max(w1,w2)>par.wk2?0:0.1;
//node.setmcsx(mecis);
//node.setmcsy(mecis);
...
if (cit->cell->lambda_celllength != 0) {
...
//cerr << "pushing back " << Neighbor(boundary_polygon->index, e.second, e.first ) << "\n";
} else {
...
//  debug_stream << "Nodes " << e.first << " and " << e.second << "\n";
...
//      debug_stream << "Cell " << c->cell << " owns Edge " << e << "\n";
...
#ifdef QDEBUG
qDebug() << "Before CleanUpCellNodeLists" << "\n";
#endif
TestIllegalWalls();
...
int no;
public:
explicit node_owners_eq(int nn) {
no=nn;
}
...
#ifdef QDEBUG
qDebug() << "\n" << "Original Boundary Polygon node indices: ";
...
<< ". The Next boundary node is: " << *itt << ((next_boundary_node->Marked()) ? " Marked" : " Unmarked") << "\n" << "\n";
#endif
break;
}
}
...
qDebug() << "OOPS! Didn't find the next boundrary node!" << "\n";
}
#endif
...
void Mesh::CellTypeDefinitor(void) {
...
double morph_radius = hull_circumference / ( 2* Pi);
//double morph_radius = boundary_polygon->ExactCircumference() / ( 2* Pi);
for (vector<Cell *>::iterator i=cells.begin(); i!=cells.end(); ++i) {
double dist_from_cen = (*i)->periclinal().Norm();
if (par.wk8 ==1) {
if ((*i)->CellType()==8) {
if (dist_from_cen < (par.wk3 * morph_radius )) {
(*i)->SetCellType(1);
} else if (dist_from_cen < (par.wk4 * morph_radius )) {
(*i)->SetCellType(0);
} else {
(*i)->SetCellType(2);
}
...
for (list<Wall *>::iterator w=walls.begin(); w!=walls.end(); ++w) {
for (int i=0; i<Cell::NChem(); i++) {
(*w)->setTransporters1(i,clean_transporters[i]);
(*w)->setNewTransporters1(i,clean_transporters[i]);
(*w)->setTransporters2(i,clean_transporters[i]);
(*w)->setNewTransporters2(i,clean_transporters[i]);
}
}
}
...
for (list<Wall *>::iterator w=walls.begin(); w!=walls.end(); w++) {
for (int i=0; i<Cell::NChem(); i++) {
(*w)->setTransporters1(i,max_transporters[i] * RANDOM());
(*w)->setNewTransporters1(i, (*w)->Transporters1(i) );
(*w)->setTransporters2(i,max_transporters[i] * RANDOM());
(*w)->setNewTransporters2(i, (*w)->Transporters1(i) );
}
}
}
...
//    f << hull[i].x << " " << hull[i].y << " " << hull[i+1].x << " " << hull[i+1].y << " " << l << "\n";
...
csv_stream << "\"Cell Index\",\"Parent Index\",\"Center of mass (x)\",\"Center of mass (y)\",\"Cell area\",\"Cell length\",\"Stiffness\"";
for (int c=0; c<Cell::NChem(); c++) {
...
csv_stream << Area() << ", " << NCells() << ", " << NNodes() << ", " << res_compactness << ", " << res_area << ", " << morph_circumference << ", " << hull_circumference << "\n";
}
/* finis */
