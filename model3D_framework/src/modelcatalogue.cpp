...
void ModelCatalogue::LoadPlugins() {
  ...
  foreach (QString fileName, pluginDir.entryList(QDir::Files)){ 
	...
	qobject_cast<SimPluginInterface *>(loader.instance())) {
      ...
      cerr << loader.errorString().toStdString().c_str() << "\n";
...
void ModelCatalogue::InstallModel(SimPluginInterface *plugin) {
  ...
  if (mainwin) {
	...
	pluginDir.cdUp();
      ...
      cerr << "pluginDir: " << pluginDir.dirName().toStdString().c_str() << "\n";
	...
	if (!pluginDir.exists(plugin->DefaultLeafML())) {
	  ...
	  //mainwin->Init(0);
	  cerr << "Default LeafML: " << plugin->DefaultLeafML().toStdString().c_str() << "\n";
