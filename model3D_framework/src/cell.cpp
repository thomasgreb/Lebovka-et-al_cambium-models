...
qDebug() << "I created an illegal wall." << "\n";
}
#endif
...
//cerr << "Constructing connections of cell " << index << "\n";

for (list<Node *>::iterator i=nodes.begin(); i!=nodes.end(); i++) {

//cerr << "Connecting node " << *i << "\n";
//cerr << "Node " << *i << "\n" << " = " << *(*i) << "\n";
...
#ifdef QDEBUG
qDebug() << "Cell " << Index() << " is doing DivideOverGivenLine" << "\n";
#endif
for (list<Node *>::iterator i=nodes.begin(); i!=nodes.end(); i++) {
...
qDebug() << "Line does not intersect with two edges of Cell " << Index() << "\n";
qDebug() << "new_node_locations.size() = " << new_node_locations.size() << "\n";
return false;
}
...
qDebug() << "Rejecting proposed division (cutting off zero area)." << "\n";
return false;
}
#endif
...
parent_info.polarization = ReduceCellAndWalls<Vector>( TRANSP0dir );
parent_info.polarization.Normalise();
parent_info.TRANSP0membrane = SumTransporters(0);
parent_info.TRANSP0endosome = Chemical(0);

//cerr << "Parent polarization before division: " << parent_info.polarization << "\n";
...
cerr << "Warning in Cell::Division: division of non-convex cells not fully implemented" << "\n";
...
#ifdef QDEBUG
qDebug() << "new_node_locations.size() = " << new_node_locations.size() <<"\n";
qDebug() << "daughter->index = " << daughter->index << "\n";
qDebug() << "cells.size() = " << m->cells.size() << "\n";
#endif
...
#ifdef QDEBUG
qDebug() << "Index of the first node: " << div_edges[i].first->Index() << "\n";
qDebug() << "Index of the second node: " << div_edges[i].second->Index() << "\n";
qDebug() << "Boundary proceeds from: " <<  div_edges[i].first->Index()
<< "to: " << (m->findNextBoundaryNode(div_edges[i].first))->Index() << "\n" << "\n";
#endif
...
#ifdef QDEBUG
qDebug() << "Considering: " << it->cell->Index() << " as a possible edge owner." << "\n";
#endif
if (it->cell->Index() != this->Index()) {
#ifdef QDEBUG
qDebug() << "Adding: " << it->cell->Index() << " to the list of edge owners." << "\n";
#endif
...
!= edge_owners.end()) {
#ifdef QDEBUG
qDebug() << "deleating: " << it->cell->Index() << " from the list of edge owners." << "\n";
#endif
...
#ifdef QDEBUG
qDebug() << "The edge owners list has: " << edge_owners.size() << " elements" << "\n";
#endif
...
//cerr << "Cell "  << index << " says: neighboring cell is " << neighbor_cell->index << "\n";
...
if (w == walls.end()) {
#ifdef QDEBUG
qDebug() << "Whoops, wall element not found...!" << "\n";
qDebug() << "Cell ID: " << neighbor_cell->Index() << "\n";
qDebug() << "My cell ID: " << Index() << "\n";
#endif
} else {
...
//cerr << "Original length is " << orig_length[i] << "\n";
if ((*w)->c1 == this ) {

//  cerr << "Cell " << (*w)->c1->Index() << " splits up wall " << *(*w) << ", into: " << "\n";
new_wall = new Wall( (*w)->n1, new_node_ind[i], this, neighbor_cell);
(*w)->n1 = new_node_ind[i];

//  cerr << "wall " << *(*w) << ", and new wall " << *new_wall << "\n";
...
AddWall(new_wall);
// cerr << "Building new wall: this=" << Index() << ", neighbor_cell = " << neighbor_cell->Index() << "\n";

neighbor_cell->AddWall( new_wall);
//cerr << "Existing wall: c1 = " << (*w)->c1->Index() << ", neighbor_cell = " << (*w)->c2->Index() << "\n";
...
//cerr << "*new_node_locations.front() = " << *new_node_locations.front() << "\n";
...
// later on.
int n=(int)((dist/Node::target_length)/ par.wk1+0.5);
...
//cerr << "Cell " << index << " has been dividing, and gave birth to Cell " << daughter->index << "\n";
...
if ( ( TINY < ua && ua < 1.-TINY ) && ( TINY < ub && ub < 1.-TINY ) ) {
//commented double numera = ((v4.x - v3.x)*(new_pos.y - v3.y) - (v4.y - v3.y)*(new_pos.x -v3.x));
//commented double numerb = ((neighbor_of_moving_node[j].x - new_pos.x)*(new_pos.y-v3.y) - (neighbor_of_moving_node[j].y- new_pos.y)*(new_pos.x - v3.x));

/* Are the wall elements coincident? */
//commented if (fabs(numera) < TINY && fabs(numerb) < TINY && fabs(denominator) < TINY) {
//commented return true;
//commented }
//commented 
//commented /* Are the wall elements parallel? */
//commented if (fabs(denominator) < TINY) {
//commented continue;
//commented }
//commented double ua = numera / denominator;
//commented double ub = numerb / denominator;
//commented //if ( ( TINY < ua && ua < 1.-TINY ) && ( TINY < ub && ub < 1.-TINY ) ) {
//commented if ( ( 0 < ua && ua < 1. ) && ( 0 < ub && ub < 1.) ) {
//cerr << "ua = " << ua << ", ub = " << ub << "\n";
return true;
}
}
...
#ifdef QDEBUG
if ((*i)->c1!=this) {
qDebug() << "Warning, bad cells boundary: " << (*i)->c1->Index() << ", " << index << "\n";
}
#endif
...
if (DeadP()) {
#ifdef QDEBUG
qDebug() << "Cell " << index << " not drawn, because dead." << "\n";
#endif
return;
}
...
p->setPolygon(pa);
//p->setPen(par.outlinewidth>=0?QPen( QColor(par.cell_outline_color),par.outlinewidth):QPen(Qt::NoPen));
//p->setPen(QPen( QColor(par.cell_outline_color), 0));
...
void Cell::DrawThick(QGraphicsScene *c)
{
CellItem* pII = new CellItem(this, c);
QPolygonF paII(nodes.size());
int ccII=0;
double radius=0;

for (list<Node *>::const_iterator n=nodes.begin(); n!=nodes.end(); n++) {
Node *i=*n;
Vector pompilio(i->x,i->y,0);
radius +=  ((this->Centroid() - pompilio).Norm());
}
radius /= this->NNodes();
double thickness = this->Chemical(2);
double erre = 1 - (thickness/radius);
for (list<Node *>::const_iterator n=nodes.begin(); n!=nodes.end(); n++) {
Node *i=*n;
paII[ccII++] = QPointF((double)((i->x - this->Centroid().x) * erre + this->Centroid().x),
(double)((i->y - this->Centroid().y) * erre + this->Centroid().y)
);
}
...
m->plugin->SetCellColor(this,&cell_color);
pII->setBrush( cell_color.lighter(150) );
//set Zvalue to 2 in order to display thickness
pII->setZValue(2);
pII->setPolygon(paII);
pII->setPen(QPen( QColor(par.cell_outline_color), 0));
...
void Cell::DrawLigni(QGraphicsScene *c)
{
CellItem* pII = new CellItem(this, c);
QPolygonF paII(nodes.size());
int ccII=0;
...
for (list<Node *>::const_iterator n=nodes.begin(); n!=nodes.end(); n++) {
Node *i=*n;
double lignin = this->Chemical(3);
Vector pompilio(i->x,i->y,0);
double radius =  ((this->Centroid() - pompilio).Norm());
double erre = 1 - (lignin/radius);
//paII[ccII++] = QPointF((double)((i->x + erre*(this->Centroid().x - i->x))),
//                       (double)((i->y + erre*(this->Centroid().y - i->y)))
//                 );
//double erre = (1- lignin);
paII[ccII++] = QPointF((double)((i->x - this->Centroid().x) * erre + this->Centroid().x),
(double)((i->y - this->Centroid().y) * erre + this->Centroid().y)
);
}
...
m->plugin->SetCellColor(this,&cell_color);
pII->setBrush( cell_color.darker(150) );
//set Zvalue to 2 in order to display lignified wall
pII->setZValue(2);
pII->setPolygon(paII);
pII->setPen(QPen(QColor("red"), 0));
...
Vector Cell::periclinal(void) const
{
return this->Centroid() - m->boundary_polygon->Centroid();
}
...
Vector from = centroid - 0.5 * width * long_axis;
Vector to = centroid + 0.5 * width *   long_axis;
...
Vector vec_flux = ReduceCellAndWalls<Vector>( TRANSP0dir );
...
//cerr << "Node " << *prev_n << " to " << *n << ", cumulative length = " << sum_length << "\n";
}
...
#ifdef QDEBUG
if (w->c1 == w->c2 ){
qDebug() << "Wall between identical cells: " << w->c1->Index()<< "\n";
}
#endif
...
{
//  cerr << "Attempting to emit " << t << ", " << chem[0] << ", " << chem[1] << "\n";
