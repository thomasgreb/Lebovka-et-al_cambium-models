...
int MainBase::Save(const char *fname, const char *format, int sizex, int sizey)
{
  ...
  if (!QString(format).contains("PDF", Qt::CaseInsensitive)) {
    ...
    canvas.setSceneRect(  QRectF(-500,-500, 1000, 1000) );
    ...
    canvas.render(&painter, QRectF(), QRectF(-750,-750, 1500, 1500)); // NB Fiddle with these arguments if relative size of PDF images is important.
