  ...
  */
  ...
  if ( ( batch || MovieFramesP() )) {
    if (!(count%par.storage_stride) ) {
      ...
      Save(fname.str().c_str(), "PNG", 1024, 1024);
    }
...
//    Vector max = this->MaxChemSide(0)->getWallVector(this);
//     //zdes nado nayti centr vectora i potomo provesti liniu megdu dvuma vectorami
//     Vector min = this->MinChemSide(0)->getWallVector(this);

//   //  Vector max1 = c->MaxChemSide(0)->getWallCenter(c);
//   //  Vector min1 = c->MinChemSide(0)->getWallCenter(c);

//     Vector res = (max - min);
//this->SetChemical(4,6.);
