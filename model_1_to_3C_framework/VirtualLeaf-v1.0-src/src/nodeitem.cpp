...
static const std::string _module_id("$Id: nodeitem.cpp,v 15d600891648 2010/06/24 13:22:58 michael $");
...
void NodeItem::setXY(const float x, const float y){
...
void NodeItem::OnClick(const Qt::MouseButton &mb, const float x, const float y, const bool clicker)
{
  if (mb == Qt::LeftButton) {
    Node *n = &getNode();
//cout<<x<<endl;
...
//второй клик то приходится по холсту и поэтому его надо обрабатывать там и координату брать оттуда
      if(clicker == false)
      {
//          cout<<"first"<<endl;
//          clicker = true;
