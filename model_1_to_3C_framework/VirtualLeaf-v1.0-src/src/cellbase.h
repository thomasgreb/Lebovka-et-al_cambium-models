...
class CellBase :  public QObject, public Vector 
{
 ...
 public:
  ...
  Wall* MaxChemSide (int chem)
   {
...

     {
         ...
         if(  ( (*w)->c1!=this ? (*w)->c1->Chemical(chem) : (*w)->c2->Chemical(chem) ) > max  )
         {
             max = ( (*w)->c1!=this ? (*w)->c1->Chemical(chem) : (*w)->c2->Chemical(chem) );
   ...
   Wall* MinChemSide (int chem)
   {
...

     {
         ...
         if(  ( (*w)->c1!=this ? (*w)->c1->Chemical(chem) : (*w)->c2->Chemical(chem) ) < min  )
         {
             min = ( (*w)->c1!=this ? (*w)->c1->Chemical(chem) : (*w)->c2->Chemical(chem) );
  ...
  int NumberOfNEighborsWithCelltype(int cellType)
  {
      int amount=0;
    ...
    {
        //      sum +=  (*w)->Length() * ( (*w)->c1!=this ? (*w)->c1->Chemical(chem) : (*w)->c2->Chemical(chem) );
//          cout<< "cell type " << (   (*w)->c1!=this ? (*w)->c1->CellType() : (*w)->c2->CellType())<<endl;
        int type = (   (*w)->c1!=this ? (*w)->c1->CellType() : (*w)->c2->CellType());
  ...
  inline int DivisionsNumber(void) const { return div_counter; }

  inline int lifeTime(void) const { return life_time; }
  inline void SetLifeTime(int lt) { life_time = lt; }
  inline void IncreaseLifeTime(void) { life_time++; }
